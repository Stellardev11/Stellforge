import express, { Request, Response } from 'express';
import { 
  fetchPopularAssets, 
  searchAssets, 
  getAssetDetails 
} from '../services/assetService';
import { 
  getSwapQuote, 
  getOrderbook, 
  findBestPath,
  getTradeHistory 
} from '../services/dexService';
import { 
  submitSignedTransaction, 
  getTransactionStatus,
  validateTransaction 
} from '../services/transactionService';

const router = express.Router();

router.get('/assets', async (req: Request, res: Response) => {
  try {
    const limit = parseInt(req.query.limit as string) || 50;
    const assets = await fetchPopularAssets(limit);
    res.json({ success: true, assets });
  } catch (error: any) {
    console.error('Error fetching assets:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to fetch assets' 
    });
  }
});

router.get('/assets/search', async (req: Request, res: Response) => {
  try {
    const query = req.query.q as string;
    
    if (!query) {
      return res.status(400).json({ 
        success: false, 
        error: 'Search query is required' 
      });
    }

    const assets = await searchAssets(query);
    res.json({ success: true, assets });
  } catch (error: any) {
    console.error('Error searching assets:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to search assets' 
    });
  }
});

router.get('/assets/:code/:issuer', async (req: Request, res: Response) => {
  try {
    const { code, issuer } = req.params;
    const asset = await getAssetDetails(code, issuer);
    
    if (!asset) {
      return res.status(404).json({ 
        success: false, 
        error: 'Asset not found' 
      });
    }

    res.json({ success: true, asset });
  } catch (error: any) {
    console.error('Error fetching asset details:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to fetch asset details' 
    });
  }
});

router.get('/quote', async (req: Request, res: Response) => {
  try {
    const { 
      fromAssetCode, 
      fromAssetIssuer, 
      toAssetCode, 
      toAssetIssuer, 
      amount,
      mode 
    } = req.query;

    if (!fromAssetCode || !fromAssetIssuer || !toAssetCode || !toAssetIssuer || !amount) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required parameters' 
      });
    }

    const quote = await getSwapQuote(
      fromAssetCode as string,
      fromAssetIssuer as string,
      toAssetCode as string,
      toAssetIssuer as string,
      amount as string,
      (mode as 'send' | 'receive') || 'send'
    );

    res.json({ success: true, quote });
  } catch (error: any) {
    console.error('Error getting quote:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to get swap quote' 
    });
  }
});

router.get('/orderbook', async (req: Request, res: Response) => {
  try {
    const { 
      sellingAssetCode, 
      sellingAssetIssuer, 
      buyingAssetCode, 
      buyingAssetIssuer,
      limit 
    } = req.query;

    if (!sellingAssetCode || !sellingAssetIssuer || !buyingAssetCode || !buyingAssetIssuer) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required parameters' 
      });
    }

    const orderbook = await getOrderbook(
      sellingAssetCode as string,
      sellingAssetIssuer as string,
      buyingAssetCode as string,
      buyingAssetIssuer as string,
      limit ? parseInt(limit as string) : 20
    );

    res.json({ success: true, orderbook });
  } catch (error: any) {
    console.error('Error fetching orderbook:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to fetch orderbook' 
    });
  }
});

router.get('/path', async (req: Request, res: Response) => {
  try {
    const { 
      fromAssetCode, 
      fromAssetIssuer, 
      toAssetCode, 
      toAssetIssuer, 
      amount 
    } = req.query;

    if (!fromAssetCode || !fromAssetIssuer || !toAssetCode || !toAssetIssuer || !amount) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required parameters' 
      });
    }

    const path = await findBestPath(
      fromAssetCode as string,
      fromAssetIssuer as string,
      toAssetCode as string,
      toAssetIssuer as string,
      amount as string
    );

    res.json({ success: true, path });
  } catch (error: any) {
    console.error('Error finding path:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to find trading path' 
    });
  }
});

router.get('/trades', async (req: Request, res: Response) => {
  try {
    const { 
      baseAssetCode, 
      baseAssetIssuer, 
      counterAssetCode, 
      counterAssetIssuer,
      limit 
    } = req.query;

    if (!baseAssetCode || !baseAssetIssuer || !counterAssetCode || !counterAssetIssuer) {
      return res.status(400).json({ 
        success: false, 
        error: 'Missing required parameters' 
      });
    }

    const trades = await getTradeHistory(
      baseAssetCode as string,
      baseAssetIssuer as string,
      counterAssetCode as string,
      counterAssetIssuer as string,
      limit ? parseInt(limit as string) : 50
    );

    res.json({ success: true, trades });
  } catch (error: any) {
    console.error('Error fetching trades:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to fetch trade history' 
    });
  }
});

router.post('/submit', async (req: Request, res: Response) => {
  try {
    const { xdr } = req.body;

    if (!xdr) {
      return res.status(400).json({ 
        success: false, 
        error: 'Transaction XDR is required' 
      });
    }

    const validation = await validateTransaction(xdr);
    if (!validation.valid) {
      return res.status(400).json({ 
        success: false, 
        error: validation.error || 'Invalid transaction' 
      });
    }

    const result = await submitSignedTransaction(xdr);
    res.json({ success: true, result });
  } catch (error: any) {
    console.error('Error submitting transaction:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to submit transaction' 
    });
  }
});

router.get('/transaction/:hash', async (req: Request, res: Response) => {
  try {
    const { hash } = req.params;
    const status = await getTransactionStatus(hash);
    res.json({ success: true, status });
  } catch (error: any) {
    console.error('Error fetching transaction status:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to fetch transaction status' 
    });
  }
});

export default router;
